...
parser.add_argument(
    "-v", "--verbose", help="increase output verbosity to DEBUG", action="store_true")
    ...
    try:
        if method_.lower() == 'get':
            r = requests.get(url_, headers=headers_,
                             timeout=(timeout_, timeout_))
        elif method_.lower() == 'post':
            r = requests.post(url_, headers=headers_,
                              data=data_, timeout=(timeout_, timeout_))
        elif method_.lower() == 'head':
            r = requests.head(url_, headers=headers_,
                              timeout=(timeout_, timeout_))
...
def get_all_rpc_ips():
    ...
    if 'result' in str(r.text):
        if WITH_PRIVATE_RPC is True:
            ...
            rpc_ips = [rpc["rpc"]
                       for rpc in r.json()["result"] if rpc["rpc"] is not None]
...
def download(url: str, count: int):
    fname = f'snapshot_downloaded{count}.tar.zst'
    ...
    try:
        ...
        logger.error(
            f'Exception in download() func. Make sure wget is installed\n{unknwErr}')
...
def main_worker():
    try:
        ...
        logger.info(
            f'RPC servers in total: {len(rpc_nodes)} | Current slot number: {current_slot}\n')
        ...
        if len(FULL_LOCAL_SNAPSHOTS) > 0:
            ...
            FULL_LOCAL_SNAP_SLOT = FULL_LOCAL_SNAPSHOTS[0].replace(
                SNAPSHOT_PATH, "").split("-")[1]
            logger.info(
                f'Found full local snapshot {FULL_LOCAL_SNAPSHOTS[0]} | {FULL_LOCAL_SNAP_SLOT=}')
        ...
        if len(json_data["rpc_nodes"]) == 0:
            logger.info(
                f'No snapshot nodes were found matching the given parameters: {args.max_snapshot_age=}')
            sys.exit()

        # sort list of rpc node by SORT_ORDER (latency)
        rpc_nodes_sorted = sorted(
            json_data["rpc_nodes"], key=lambda k: k[SORT_ORDER])
        ...
        logger.info(
            f'All data is saved to json file - {SNAPSHOT_PATH}/snapshot.json')
        ...
        for i, rpc_node in enumerate(json_data["rpc_nodes"], start=1):
            ...
            if BLACKLIST != ['']:
                if any(i in str(rpc_node["files_to_download"]) for i in BLACKLIST):
                    logger.info(
                        f'{i}\\{len(json_data["rpc_nodes"])} BLACKLISTED --> {rpc_node}')
                    continue

            logger.info(
                f'{i}\\{len(json_data["rpc_nodes"])} checking the speed {rpc_node}')
            if rpc_node["snapshot_address"] in unsuitable_servers:
                logger.info(
                    f'Rpc node already in unsuitable list --> skip {rpc_node["snapshot_address"]}')
                continue

            down_speed_bytes = measure_speed(
                url=rpc_node["snapshot_address"], measure_time=SPEED_MEASURE_TIME_SEC)
            ...
            if down_speed_bytes < MIN_DOWNLOAD_SPEED_MB * 1e6:
                ...
                logger.info(
                    f'Suitable snapshot server found: {rpc_node=} {down_speed_mb=}')
                for path in reversed(rpc_node["files_to_download"]):
                    ...
                    if 'incremental' in path:
                        r = do_request(
                            f'http://{rpc_node["snapshot_address"]}/incremental-snapshot.tar.bz2', method_='head', timeout_=2)
                        if 'location' in str(r.headers) and 'error' not in str(r.text):
                        ...
                        best_snapshot_node = f'http://{rpc_node["snapshot_address"]}{path}'
                    logger.info(
                        f'Downloading {best_snapshot_node} snapshot to {SNAPSHOT_PATH}')
                    # COUNT = COUNT + 1
                    download(url=best_snapshot_node, count=test)
                    test = test + 1
                return 0
...
while NUM_OF_ATTEMPTS <= NUM_OF_MAX_ATTEMPTS:
    ...
    logger.info(
        f'Attempt number: {NUM_OF_ATTEMPTS}. Total attempts: {NUM_OF_MAX_ATTEMPTS}')
    NUM_OF_ATTEMPTS += 1
