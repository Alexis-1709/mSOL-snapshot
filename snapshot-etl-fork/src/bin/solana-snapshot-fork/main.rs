...
use solana_snapshot_fork::archived::ArchiveSnapshotExtractor;
use solana_snapshot_fork::parallel::AppendVecConsumer;
use solana_snapshot_fork::unpacked::UnpackedSnapshotExtractor;
use solana_snapshot_fork::{AppendVecIterator, ReadProgressTracking, SnapshotExtractor};
...
impl SupportedLoader {
    ...
    fn new_file(
        path: &Path,
        progress_tracking: Box<dyn ReadProgressTracking>,
    ) -> solana_snapshot_fork::Result<Self> {
        Ok(if path.is_dir() {
