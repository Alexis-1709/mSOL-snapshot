...
var dataOrcaWhirlpoolsHolders = []
var dataTulipHolders = []
var dataMercurialHolders = []
var dataSaberHolders = []
...
function orcaWhrilpools(){
    const whirlpool_msol_usdc = db.prepare(`SELECT token_a, token_b, sqrt_price FROM whirlpool_pools WHERE pubkey= ?`).all("AiMZS5U3JMvpdvsr1KeaMiS354Z1DeSg5XjA4yYRxtFf");
    const result = db.prepare(`SELECT orca.price_lower, orca.price_upper, orca.liquidity, token_account.owner FROM orca, token_account WHERE orca.position_mint=token_account.mint AND orca.pool= ?`).all("AiMZS5U3JMvpdvsr1KeaMiS354Z1DeSg5XjA4yYRxtFf");
    result.forEach((row) => {
        ...
        if(dataOrcaWhirlpoolsHolders[row.owner] == undefined){
            dataOrcaWhirlpoolsHolders[row.owner] = orca.DecimalUtil.fromU64(amounts.tokenA, 9).toNumber();
        }else{
            dataOrcaWhirlpoolsHolders[row.owner] += orca.DecimalUtil.fromU64(amounts.tokenA, 9).toNumber();
        }
...
function getTulipBalance(){
    const result = db.prepare(`SELECT token_account.owner, token_account.amount, account.pubkey FROM token_account, account WHERE token_account.mint= ? AND token_account.owner=account.pubkey AND account.owner= ? AND token_account.amount>0 ORDER BY token_account.amount desc`).all(["8cn7JcYVjDZesLa3RTt3NXne4WcDw9PdUneQWuByehwW", "11111111111111111111111111111111"]);
    result.forEach((row) => {
        if(dataTulipHolders[row.owner] == undefined){
            dataTulipHolders[row.owner] = row.amount*toUi;
        }else{
            dataTulipHolders[row.owner] += row.amount*toUi;
        }
...
function getOrcaAquafarmsBalance(){
    const mSOL_USDT_supply = db.prepare(`SELECT supply FROM token_mint WHERE pubkey= ?`).all("Afvh7TWfcT1E9eEEWJk17fPjnqk36hreTJJK5g3s4fm8")[0].supply*Math.pow(10, -6);
...
function getMercurialBalance(){
    const mSOL_2Pool_supply = db.prepare(`SELECT supply FROM token_mint WHERE pubkey= ?`).all("7HqhfUqig7kekN8FbJCtQ36VgdXKriZWQ62rTve9ZmQ")[0].supply*Math.pow(10, -12);
    const mSOL_in_liq = db.prepare(`SELECT amount FROM token_account WHERE pubkey= ?`).all("GM48qFn8rnqhyNMrBHyPJgUVwXQ1JvMbcu3b9zkThW9L")[0].amount*toUi;
    ...
    result.forEach((row) => {
        if(dataMercurialHolders[row.owner] == undefined){
            dataMercurialHolders[row.owner] = row.amount*toUi*mSOL_per_LP;
        }else{
            dataMercurialHolders[row.owner] += row.amount*toUi*mSOL_per_LP;
        }
...
function getSaberBalance(){
    const mSOL_SOL_supply = db.prepare(`SELECT supply FROM token_mint WHERE pubkey= ?`).all("SoLEao8wTzSfqhuou8rcYsVoLjthVmiXuEjzdNPMnCz")[0].supply*toUi;
    const mSOL_in_liq = db.prepare(`SELECT amount FROM token_account WHERE pubkey= ?`).all("9DgFSWkPDGijNKcLGbr3p5xoJbHsPgXUTr6QvGBJ5vGN")[0].amount*toUi;
    ...
    result.forEach((row) => {
        if(dataSaberHolders[row.owner] == undefined){
            dataSaberHolders[row.owner] = row.amount*toUi*mSOL_per_LP;
        }else{
            dataSaberHolders[row.owner] += row.amount*toUi*mSOL_per_LP;
        }
...
function parseData(){
    owners.forEach((owner) => {
        data[owner] = {
            ...
            OrcaWhirlpoolBalance : dataOrcaWhirlpoolsHolders[owner] ? dataOrcaWhirlpoolsHolders[owner] : 0,
            TulipBalance : dataTulipHolders[owner] ? dataTulipHolders[owner] : 0,
            MercurialBalance : dataMercurialHolders[owner] ? dataMercurialHolders[owner] : 0,
            SaberBalance : dataSaberHolders[owner] ? dataSaberHolders[owner] : 0,
        }
    })
}
